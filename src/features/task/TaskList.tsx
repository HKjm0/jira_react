...
const TaskList: React.FC = () => {
  ...
  const columns = tasks[0] && Object.keys(tasks[0]); // Tasks(READ_TASK)のkey値を文字列で取得する
  ...
  const handleClickSortColumn = (column: keyof READ_TASK) => {
    const isDesc: boolean =
      column === state.activeKey && state.order === "desc";
        ...
        <Table size="small" className={classes.table}>
          <TableHead>
            <TableRow>
              ...
              <TableCell /*削除、編集ボタン用に空のテーブルセルを追加*/
              ></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
